...
struct Page *addTupleToPage(struct Page *page, const struct Tuple *tuple) {
...
// Function to display the number of pages and tuples in each page
void displayPages(const struct Page *page) {
    const struct Page *currentPage = page;
    while (currentPage != NULL) {
        printf("Page %u:\n", currentPage->header.pageNo);
        printf("Number of Tuples: %u\n", currentPage->header.nItems);
        printf("--------------\n");
        currentPage = currentPage->header.nextPageNo == 0 ? NULL : (struct Page *)currentPage->header.nextPageNo;
    }
}
...
int main() {
    ...
    // Specify the total number of tuples to add
    int totalTuples = 100; // Change this to the desired total number of tuples

    // Add tuples until the total number of tuples is reached
    for (int i = 0; i < totalTuples; i++) {
        ...
        struct Page *currentPage = addTupleToPage(&page, &tuple);
        if (currentPage == NULL) {
            printf("Error adding tuple to page.\n");
            break;
        }
    }

    // Display the number of pages and tuples in each page
    displayPages(&page);
